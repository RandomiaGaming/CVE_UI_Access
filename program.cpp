...
void PrintLastError() {
	...
	// Allocate a narrow string with enough space for errorMessage + a null character.
	CHAR* errorMessageNarrow = new CHAR[size + 1];

	// Use a system API to convert the wide string to a narrow string.
	BOOL usedDefaultChar = false;
	int result = WideCharToMultiByte(CP_UTF8, 0, errorMessage, -1, errorMessageNarrow, size + 1, NULL, &usedDefaultChar);

	// Print the narrow version of errorMessage to the console.
	cerr << "Error: " << errorMessageNarrow;
